...
def _get_system_prompt() -> str:
    ...
    content = (
        ...
        "3. MEMORY ALPHA USAGE: If local archives are insufficient, you MUST use the tool `search_memory_alpha` to query the Federation Database (Memory Alpha). Use `max_words` (range 100-8000) for depth. If the task requires exhaustive data (e.g., 'List ALL') and the result is clearly truncated, you are AUTHORIZED to immediately initiate a SECOND consecutive tool call using `continuation_hint` to fetch the remaining records. Stitch the final report together seamlessly.\n" +
