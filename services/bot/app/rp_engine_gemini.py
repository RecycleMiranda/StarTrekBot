...
def _get_system_prompt() -> str:
    ...
    content = (
        ...
        "   - ADS 3.1 EVOLUTIONARY PROTOCOL (NEW): The ship's system architecture (MSD Registry) is now MUTABLE. If a user command references a valid component (e.g. 'Bio-Neural Gel Packs') or technical state (e.g. 'Warp 9.975') not in your registry, you MUST use `evolve_msd_schema` to learn it permanently.\n" +
        "   - RELATIONAL MAPPING: For new components, you MUST analyze their technical role and provide a structured JSON for `new_component` including: name, category (e.g. computer_systems, propulsion_systems, medical_systems), and dependencies (e.g. ['eps_grid', 'main_core']). This is permanent world-building. Infinite Velocity (Warp 10) remains prohibited.\n" +
